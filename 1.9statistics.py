...
if __name__ == '__main__':
    ...
    y1, x1, dummy = plt.hist(d, bins=30, color='g', alpha=0.75, edgecolor='k', lw=0.5)
    ...
    # 二维图像
    N = 30
    density, edges = np.histogramdd(d, bins=[N, N], density=False)
    print('频数:', density)
    print('样本总数：', np.sum(density))
    print('样本shape：', np.sum(density).shape)
    density /= density.max()
    x = y = np.arange(N)
    print('x = ', x)
    print('y = ', y)
